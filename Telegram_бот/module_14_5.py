...
async def set_username(message, state):
    user_true=is_included(message.text)
    if user_true:
...
async def set_age(message, state):
    await state.update_data(age=message.text)
    user_add = await state.get_data()
    add_user(user_add['username'], user_add['email'], user_add['age'])
    await state.finish()
    user_true = is_included(user_add['username'])
    if user_true:
        await message.answer(f'Пользователь {user_add["username"]} добавлен')
